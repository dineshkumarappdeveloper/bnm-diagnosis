                    ...
                    is UpdateCheck.Available -> Column {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Button(onClick = {
                                ...
                                // The release may carry the BNM Lab filename or the
                                // pre-rename BNM Lab one, so take the name from
                                // the URL that was actually resolved rather than
                                // guessing — the checksum line must match it exactly.
                                val assetFileName = url.substringAfterLast('/')
                                downloading = true
                                progress = 0f
                                scope.launch {
                                    ...
                                    val sha = run {
                                        ...
                                        try {
                                            UpdateChecker.fetchChecksum(
                                                c, s.release.checksumsUrl, assetFileName,
                                            )
                                        } finally { c.close() }
                                    }
