...
/**
 * Email-only auth stub. BNM Lab signs in with email/password against
...
class FirebaseAuthManager {
    ...
    suspend fun signInWithGoogle(): Result<AdminUser> =
        Result.failure(Exception("Google sign-in is not available in BNM Lab"))

    suspend fun signInWithApple(): Result<AdminUser> =
        Result.failure(Exception("Apple sign-in is not available in BNM Lab"))
