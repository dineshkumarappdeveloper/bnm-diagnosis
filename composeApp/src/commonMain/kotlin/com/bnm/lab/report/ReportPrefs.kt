...
class ReportPrefs {
    ...
    /**
     * Where finished reports are filed as PDFs, browsable from the file
     * manager without opening the app. Blank until first read, when it takes
     * the platform's default (Documents on a desktop).
     */
    var reportsDir: String
        get() = s.getString(K_REPORTS_DIR, "").ifBlank { defaultReportsDir() }
        set(v) = s.putString(K_REPORTS_DIR, v.trim())

    /** File every finished report into [reportsDir]. On by default: a lab that
     *  never opens the folder loses nothing, and one that needs it has it. */
    var archiveReports: Boolean
        get() = s.getBoolean(K_ARCHIVE, true)
        set(v) = s.putBoolean(K_ARCHIVE, v)
    ...
    private companion object {
        ...
        const val K_REPORTS_DIR = "report_archive_dir"
        const val K_ARCHIVE = "report_archive_on"
    }
}
