...
| Flag | Meaning |
| --- | --- |
| `--id <ids>` | specimen id: one, a comma list, or a pattern — `ACC-S1-000{1..5}` expands to five, leading zeros kept |
...
| `--qc` | a QC material run (Mindray `MSH-11 = Q`); the app ACKs and ignores it |
| `--no-histograms` | numbers only |
...
| --- | --- | --- |
| `--truncated` | cuts the frame at ~55% and closes | bytes counted, **frames 0**, log row "Connection closed with N unframed bytes (ignored)" |
...
| `--burst <n>` | n connections at once | n results, none lost |
| `--hang` | connects, sends nothing, holds the socket (`--interval` seconds, default 30) | the peer address, bytes 0, frames 0 |
...
| `random` | a plausible patient drawn anywhere across the ranges |
...
1. **The link itself.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --hang --interval 10
   ```
...
2. **A result with no order — the claim queue.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --id NOT-AN-ORDER
   ```
...
3. **A result on a real order.** Register a patient and a CBC in BNM Lab, note
   the accession, then:
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --id ACC-S1-00042 --image
   ```
...
4. **Nothing keyed on the analyzer.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --no-specimen
   ```
...
5. **Half a frame.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --truncated
   ```
...
6. **A dribbled frame.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --id ACC-S1-00043 --slow-chunks 40 --image
   ```
...
7. **A critical, end to end.**
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --id ACC-S1-00044 --profile critical
   ```
...
8. **A small run**, to watch it work at pace:
   ```bash
   java -jar analyzer-sim.jar mindray --host <lab PC> --id ACC-S1-000{1..5} --interval 3
   ```

For a Mispa install, the same list with `mispa --serial COM3` in place of
`mindray --host …`. Step 1 becomes "does the port open at all" — the simulator
names the adapter it found, and if it cannot open the port, that is the cable
or a terminal program holding it.
...
- **QC on the Mispa.** That format has no processing-id field, so `--qc` has
  nothing to set and a QC material arrives as another sample. The CLI says so
