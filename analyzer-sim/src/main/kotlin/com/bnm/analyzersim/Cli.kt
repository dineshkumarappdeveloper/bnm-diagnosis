...
object Cli {
     ...
     *
     * PUBLIC because it is the ONE gate: the window (`:analyzer-sim-ui`) builds
     * its own [Options] from a form and runs them through here before it will
     * send, so the live-lab refusal and the serial ones cannot hold on the
     * command line and quietly not hold in a UI.
     */
    fun validate(o: Options) {
        if (o.usesSerial) {
